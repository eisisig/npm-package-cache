...
export default class Manager {
	...
	static getFileHash () {
		...
		if ( fs.existsSync(isShrinkWrapped) ) {
			let hashedFile = this.hashFile(isShrinkWrapped);
			Logger.info('Using npm-shrinkwrap.json - ' + hashedFile)
			return hashedFile;
		} else {
			let hashedFile = this.hashFile(path.resolve(process.cwd(), 'package.json'));
			Logger.info('Using pacakge.json - ' + hashedFile)
