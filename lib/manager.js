...
Manager.prototype.install = function ( opts ) {
	...
	if ( !opts.forceFetch && archive ) {
		...
		this.npmInstall(opts.forceFetch);
	}

};

Manager.prototype.requirements = function () {
	console.log('Manager: requirements');
};
...
Manager.prototype.search = function ( hash ) {
	...
	var pattern = config.cacheDir + '/' + hash + '.7z'.replace(/\\/gi, '/');

	Logger.info('Searching for file: ' + hash);
...
Manager.prototype.npmInstall = function ( force ) {
